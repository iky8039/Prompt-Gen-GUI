...
class PromptBuilderApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Interactive Prompt Builder")
        ...
        self.create_btn(self.btn_frame, "Load...", self.load_keywords_from, COLORS["secondary"])
    ...
    def add_keyword(self):
        new_kw = self.search_var.get().strip()
        if new_kw and new_kw not in self.my_keywords:
            self.my_keywords.append(new_kw)
            self.auto_save_keywords()
            self.search_var.set("")
            self.update_keyword_ui()

    def reset_keywords(self):
        if messagebox.askyesno("Reset", "Revert to default keyword list?"):
            self.my_keywords = self.default_keywords.copy()
            self.auto_save_keywords()
            self.update_keyword_ui()
...
if __name__ == "__main__":
    try:
        ...
        print("Colab execution detected. GUI unavailable.")
