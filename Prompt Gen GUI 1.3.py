...
__version__ = "1.4"
APP_NAME = "Interactive Prompt Builder"
...
class PromptBuilderApp:
    def __init__(self, root):
        ...
        # --- エイリアス対応辞書の読み込み ---
        self.alias_dict = self.load_alias_dict()

        # --- エイリアスjsonの全英語キーワードをmy_keywordsに初期追加 ---
        if self.alias_dict:
            for k in self.alias_dict.keys():
                if k not in self.my_keywords:
                    self.my_keywords.append(k)
        ...
        if search_term:
            ...
            for kw in self.my_keywords:
                ...
                if search_term in kw.lower():
                    ...
                    continue
                # エイリアスでヒット
                aliases = self.alias_dict.get(kw, [])
                if any(search_term in alias.lower() for alias in aliases):
                    filtered_keywords.append(kw)
        else:
            filtered_keywords = self.my_keywords
